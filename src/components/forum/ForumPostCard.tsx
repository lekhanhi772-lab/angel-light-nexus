...
import { Heart, MessageCircle, Clock, MoreHorizontal, Edit, Trash2, Pin, Gift } from 'lucide-react';
...
export function ForumPostCard({ post, onLike, onDelete, onEdit, authorWalletAddress }: ForumPostCardProps) {
    ...
    <Web3Provider>
      <>
        ...
        >
            ...
            >
              {post.author?.avatar_url ? (
                <img
                  ...
                  className="w-10 h-10 rounded-full border-2 transition-transform group-hover:scale-105"
                  style={{ borderColor: '#DAA520' }}
                />
              ) : (
                <div
                  className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold transition-transform group-hover:scale-105"
              ...
              <div>
                <p className="font-semibold text-sm group-hover:underline" style={{ color: '#8B6914' }}>
          ...
          {/* Footer: Likes + Comments + Gift */}
          <div className="flex items-center gap-3 pt-3 border-t" style={{ borderColor: 'rgba(218, 165, 32, 0.2)' }}>
            <button
            ...
            {/* Gift button - only show if not author */}
            {!isAuthor && user && (
              <button
                onClick={handleGiftClick}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-full transition-all hover:scale-105"
                style={{ background: 'rgba(218, 165, 32, 0.1)' }}
                title={t('gift.tip_author', 'Tặng quà cho tác giả')}
              >
                <Gift className="w-4 h-4" style={{ color: '#B8860B' }} />
              </button>
            )}
          </div>
        </div>
        ...
        <GiftDialog
          ...
          recipientName={post.author?.display_name || t('forum.anonymous')}
          postId={post.id}
        />
      </>
    </Web3Provider>
